...
file_path = 'template_dialogues/'
def selection_phrase(test_data, name_file):

    with open(f"{file_path}{name_file}.txt", "r", encoding="utf-8") as f1, open(f"{file_path}{name_file}_json.txt", "r", encoding="utf-8") as f2:
        lines1 = f1.readlines()
        lines2 = f2.readlines()
    list_ind = []
...
class Dialogue:
    ...
    def start(self):
        ...
        action_dm_pred = []
        outputs_nlg = []
        test_data = selection_phrase(test_data, "input")
        count = 0   
